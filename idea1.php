...
if(isset($_POST['submit']))
{
  ...
  $sql="insert into tag_comment(tag_id,p_id,u_id,comment) VALUES ('$tid','$cid','$uid','$cmt')";
...
<a href='evaluation.php' class='breadcrumb-link'>Tagline</a>
				&nbsp;/&nbsp;
...
<?php 
 @$query2=mysql_query("select * from tag_comment inner join user on tag_comment.u_id=user.u_id where tag_comment.p_id='$cid' and tag_comment.tag_id='$tid'");
...
<?php
  ...
  $sql121="select sum(tag_vote) as v from pro_tag where sub_id='$cid' and tag_id='$tid'";
$query121=mysql_query($sql121);
while($row121=mysql_fetch_array($query121))
{
$vote=$row121['tag_vote'];
$vote1=$row121['v'];
}
?>
...
<?php
/*$query="select * from pro_tag where p_id='$cid'";
...
<h3>Comments
<?php
//echo  "select * from naming_comment inner join user on naming_comment.u_id=user.u_id where naming_comment.p_id='$cid'";
$query2=mysql_query("select * from tag_comment inner join user on tag_comment.u_id=user.u_id where tag_comment.p_id='$cid' and tag_comment.tag_id='$tid'");
@$nr2=mysql_num_rows($query2);
if($nr2==0)
{
...
<?php 
echo "<table>";
//echo "select * from tag_comment inner join user on tag_comment.u_id=user.u_id where tag_comment.p_id='$cid' and tag_comment.tag_id='$tid'";
@$query3=mysql_query("select * from tag_comment inner join user on tag_comment.u_id=user.u_id where tag_comment.p_id='$cid' and tag_comment.tag_id='$tid'");
