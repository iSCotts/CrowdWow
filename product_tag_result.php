...
<a href='influnce.php' class='breadcrumb-link'>Influence</a> 
				&nbsp;/&nbsp;
<span class='breadcrumb-current'>Branding / Product Tagline</span> 
</p> 
</div> 
</div> 
...
<p>Donâ€™t forget to do a search for potential trademark infringement before submitting your name idea. This can be as simple as running a google search on your name submission. While we wonâ€™t disqualify a name that belongs to another product, store, etc., it might have an effect on our decision.</p> 
...
<?php
 $s2="select max(tag_vote) as m from pro_tag where sub_id='$id1'";
$q2=mysql_query($s2);
while($rw2=mysql_fetch_array($q2))
{
 $mvote=$rw2['m'];

 $s3="select * from pro_tag where sub_id='$id1' and tag_vote='$mvote'";
$q3=mysql_query($s3);
while($r3=mysql_fetch_array($q3))
{
$tag_name=$r3['tag_name'];

?>
<h3>
<a href="idea.php?i_id=<?php echo $id1; ?>" style="text-decoration:none;"><?php echo $tag_name; ?></a> <span class="more">&raquo;</span>
</h3>
<?php }} ?>
<cite>
<?php
$s4="SELECT * FROM user inner JOIN pro_tag on user.u_id=pro_tag.u_id where pro_tag.sub_id='$id1'";
$q4=mysql_query($s4);
...
?>
<h3 style="width:248px;"><a href="#" style="text-decoration:none;"><img src="images/icon_small_comments1.png"/>Comments(<?php if($nr1==0){echo "0";}else { echo $nr1; }  ?>)&nbsp;&nbsp;</a>Total Votes <font size=3 color=blue>(<?php echo $row['tag_vote']; ?>)</font></h3>
