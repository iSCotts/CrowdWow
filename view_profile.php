...
echo "<h1 style='font-weight: bold; color: rgb(0, 51, 255);'>Welcome ". $toname."</h1>";
?>
</a>
</h0>
</div>
...
@$sql11="SELECT shop.s_name, product.p_name, product.p_image, product.p_type, product.p_title, product.p_cur_price, product.p_pre_price, product.p_status, product.p_id, product.s_id, product.p_cur_price, submit_idia.sub_id,submit_idia.u_id 
FROM shop
INNER JOIN product ON shop.s_id = product.s_id
INNER JOIN submit_idia
ON submit_idia.sub_id =product.p_id and submit_idia.u_id='$usid' or submit_idia.u_id='$userid' ";
$query11=mysql_query($sql11);
$nr11=mysql_num_rows($query11);
?>
...
<h3><a href='#'>Props </a></h3>
	...
	*/
	$query = "SELECT COUNT(*) as num FROM props where u_id='$userid'";
	@$total_pages = mysql_fetch_array(mysql_query($query));
	  $total_pages = $total_pages[num];
	
	/* Setup vars for query. */
	$targetpage = "view_profile.php"; 	//your file name  (the name of this file)
...
<?php
 $usid=$_GET['$id'];
 @$userid=$_SESSION['u_id']; 
$sql4="SELECT user.u_firstname, user.u_lastname,props.u_id,user.u_id,props.prop_comment,props.prop_id ,props.date FROM user inner JOIN props ON user.u_id=props.u_id WHERE props.tou_id = '$userid' ORDER BY props.prop_id DESC LIMIT $start, $limit";
