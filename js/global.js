...
function openPreviewMultiLingual(){
        ...
        if(loc.indexOf("surveyconsole") > 0) {
                ...
                loc = "../akira/TakeSurveyf038.html?id=231362&amp;responseCheck=false";
        }
...
function RaterControl(promptBoxId, strings, questionId, responseUri) {
  var imagePath = "../images/ratercontrol/index.html";
...
function ajaxSubmitFormObjectFinal(divID, url, htmlForm, evaluateJS) {
    ...
    r.onreadystatechange = function () {
        ...
        if (evaluateJS) {
          ...
          if (r.responseText.indexOf("ajax-eval-code") > 0) {
            var ary = r.responseText.split("<ajax-eval-code>");
            element.innerHTML = ary[0];
            eval(ary[1].split("</ajax-eval-code>")[0]);
          } else if (r.responseText.indexOf("//JavaScript") > 0) {
            eval(r.responseText);
          } else {
...
function ajaxInlinePopup(id, url) {
    ...
    dataContainer.innerHTML = '<div style="padding:10px;" align="center"><img src="../images/waiting.gif"><span style="vertical-align:middle;">Loading...</span></div>';

    leftPx = 220 + document.body.scrollLeft;
    container.style.left = leftPx + 'px';
    topPx = 100 + document.body.scrollTop;
...
function imageQuestionHilite(id, ary) {
    ...
    if (inp && inp.checked) {
        ...
        setStyle(div, 'background: #ffffcc;cursor:pointer;cursor:hand;');
        div.onmouseout=function() {return false;};
    } else {
        setStyle(div, 'background: #BDBDBD;cursor:pointer;cursor:hand;');
...
function salesChatPopup(livePersonURL, showMin, chatOverride) {
    ...
    document.write('<div id="SalesChatPopup" style="background:url(\'/images/sales-chat-popup-bg.gif\') ' + 
                   ...
                   ';left:40%;z-index:10;">' +
                   '<div align="right" style="padding-top:15px;padding-bottom:0px;padding-right:10px;">' +
                   ...
                   '  <a href="' + livePersonURL +  '" ><img src="images/sales-chat-popup-blank.gif" border="0" style="background-repeat:no-repeat"></a>' +
                   '</div>' +
                   
                   '</div>' +
                   '<div id="SalesChatPopupMin" style="background:url(\'images/sales-chat-popup-min.gif\') ' + 
                   'no-repeat;width:330px;height:76px;right:0;bottom:0;position:absolute;z-index:10;display:none;cursor:pointer;cursor:hand;"' +
                   ' onclick="document.cookie=\'minimizeSalesPopup=false;path=/;\' + document.cookie;' + 
                   'hideDiv(\'SalesChatPopupMin\');showDiv(\'SalesChatPopup\');animateDivToCenter(\'SalesChatPopup\',0, 120);deleteCookie(\'noSalesPopup\');"></div>');
        ...
        document.onscroll = function () {
            document.getElementById('SalesChatPopup').style.top = document.body.scrollTop + 120;
            document.getElementById('SalesChatPopupMin').style.bottom = -document.body.scrollTop;
        };
}
...
function Slider() {
    ...
    this.printLabel = function () {
        var dx = (parseInt(this.SLIDE_ELEMENT.style.marginLeft) ? parseInt(this.SLIDE_ELEMENT.style.marginLeft) : 0) + 0;
        var pos = 0;
        if (dx > 0) {
            pos = this.WIDTH / this.POINTS.length;
        ...
        this.POSITION= (pos >= this.POINTS.length ? this.POINTS.length-1 : pos);  
        var sliderWidth = ((this.SLIDE_ELEMENT.clientWidth / 4)-5);   
        this.SLIDE_ELEMENT.innerHTML = "<div align='center' class='QuestionText' style='background:#fff;position:absolute;width:60px;" + 
        "margin-top:35px;margin-left:" + sliderWidth + ";" + 
        "border:1px solid #ccc;font-weight:bold;cursor:pointer;cursor:hand;'>" + this.POINTS[this.POSITION] + "</div>";
