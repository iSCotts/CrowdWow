...
$uid=$_SESSION['email'];
$s="select * from user where u_email='$uid'";
$q=mysql_query($s);
$r=mysql_fetch_array($q);
$id=$r['u_id'];
$date=date("d/m/Y");

 $sql="insert into submit_idia (u_id,project_type,desc_idea,category,desc_problem,desc_solution,invest,attach_files,start_date ) VALUES ('$id','U','$didea','$category','$d_prob','$d_solut','$bal','$attechfile','$date')";
...
$s1="select * from user where u_email='$uid'";
$q1=mysql_query($s1);
$r1=mysql_fetch_array($q1);
$balance=$r1['u_balance'];
if($balance=="")
{
$sql5="update user set u_balance='$bal' where u_email='$uid'";
$query5=mysql_query($sql5);
}
else
{
$bal1=$bal+$balance;
$sql6="update user set u_balance='$bal1' where u_email='$uid'";
...
window.location.href="idea_stage2.php?sid=<?php echo $sub_id; ?>";
</script>
<?php
...
<a href='invent.php' class='breadcrumb-link'>Invent</a>
			&nbsp;/&nbsp;
<span class='breadcrumb-current'>Idea stage 1</span>
</p>
...
<form action="" enctype="multipart/form-data" id="pitchForm" method="post" style="margin-top:0px;"><div class="form-controls box mSmaller filled-bright" style="width:740px">
...
<textarea cols="40" id="idea_title" name="idea_title" rows="20"></textarea>
<div class="clear"></div>
...
if($i==0 && $j==0)
{
echo "<input type='radio' value='".$row['s_name']."' checked='checked'  name='shopradio' />";
}
else
{
...
<textarea cols="40" id="idea_problem" name="idea_problem" rows="20"></textarea>
...
<textarea cols="40" id="idea_solution" name="idea_solution" rows="20"></textarea>
...
<p class="item-title">Proposed Cost</p>
<p class="item-subtitle">Now tell the expected cost here</p>
