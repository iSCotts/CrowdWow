...
window.location.href="umanage_project.php?dm=msg";
</script>
<?php
}
...
?>
<script type="text/javascript">
window.location.href="umanage_project.php?sm=msg";
</script>
<?php
}
?>
...
function deleteid(did) 
{
	...
	if (answer==true)
	{
	window.location.href = "umanage_project.php?did="+did;
	return true;
	}
	else
	{
...
if (r==true)
  {
   window.location.href='umanage_project.php?sid='+id+'&mode='+mode;
  }
else
  {
   return false;			
...
{
var googlewin=dhtmlwindow.open("googlebox", "iframe", "ueditproject.php?aid="+id, "Edit Ad Details", "width=500px,height=400px,resize=1,scrolling=1,center=1")
	...
	function search1(id)
{
   ...
   window.location.href='umanage_project.php?sid='+id;
...
<section id="main" class="column">
	...
	<?php echo "successfully status change"; ?>
		...
		<header>
	<h3 class="tabs_involved">User Project Management</h3>
	...
	*/
	$query = "SELECT COUNT(*) as num FROM submit_idia where project_type='U'";
	@$total_pages = mysql_fetch_array(mysql_query($query));
	  $total_pages = $total_pages[num];
	
	/* Setup vars for query. */
	$targetpage = "umanage_project.php"; 	//your file name  (the name of this file)
...
//else
//{
$result = mysql_query("SELECT * from submit_idia where project_type='U' order by sub_id desc LIMIT $start, $limit");
...
else if($t>$t1 && $t<$t2 && $t<$t3 && $t<$t4 && $status=='0' && $sp='evaluation')
{
